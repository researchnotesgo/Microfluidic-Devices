...
# ✅ 0.2 mm outward, wall_ratio_REMOVED = 0.30; bold ~70% annotations
# ✅ GPT-5 SVG primary (10s watchdog), local fallback
# ✅ OpenSCAD→Trimesh union fallback; watertight checks; CSV + prompt logs

import os, math, time, uuid, hashlib, tempfile, subprocess, csv, re, textwrap, struct
import numpy as np
...
DEFAULT = {"w":600, "internal_d":400, "r0":200, "s":2100, "loops":5,
           "h":1900, "v":1500, "angle":40}

# (API removed) SVGs are generated locally.
...
# ---------- STL hash (script 1 logic) ----------
# Use direct file MD5 (no canonical rewrite).
def canonical_md5(stl_path):
    return md5_of_file(stl_path)


# ---------- circular sweep ----------
def canonicalize_stl_inplace(stl_path, decimals=6):
    """Rewrite STL deterministically so file-level MD5 is stable across runs.

    - Loads STL into trimesh
    - Rounds vertex coordinates (mm) to `decimals`
    - Sorts faces by a stable lexicographic key
    - Writes a binary STL with a constant 80-byte header
    """
    ...
    tris = v[f]  # (n,3,3)
    # Stable face sort key from flattened triangle vertices
    keys = np.round(tris.reshape(len(tris), -1), decimals=decimals)
    ...
    out = stl_path
    header = b"CANONICAL_STL_MD5_STABLE".ljust(80, b" ")
    with open(out, "wb") as fp:
        fp.write(header[:80])
        fp.write(struct.pack("<I", tris.shape[0]))
        for i in range(tris.shape[0]):
            fp.write(struct.pack("<3f", *n[i]))
            fp.write(struct.pack("<3f", *tris[i, 0].astype(np.float32)))
            fp.write(struct.pack("<3f", *tris[i, 1].astype(np.float32)))
            fp.write(struct.pack("<3f", *tris[i, 2].astype(np.float32)))
...
def sweep_hollow_tube(points_mm, outer_d_mm, wall_mm, segments=64):
    ...
    return m
def sweep_hollow_tube_volume(points_mm, outer_d_mm, inner_d_mm, segments=64):
    """
    Watertight *volume* of a hollow tube with a through-lumen.
    - Outer diameter = outer_d_mm
    - Inner diameter = inner_d_mm
    Ends are closed with annulus rims (hole remains open), so slicers see a true hollow channel.
    """
    P = np.asarray(points_mm, float)
    if len(P) < 2:
        raise ValueError("Need at least 2 points to sweep a tube.")
    R = max(float(outer_d_mm) / 2.0, 1e-6)
    r = max(float(inner_d_mm) / 2.0, 1e-6)
    if r >= R:
        # keep a minimal wall if user gives invalid values
        r = 0.9 * R
    ...
    for i in range(len(P)):
        n, b, c = Nn[i], Bb[i], P[i]
        circle = (n[None,:]*ct[:,None] + b[None,:]*st[:,None])
        Vout.append(c + circle * R)
        Vin.append(c + circle * r)
    Vout = np.vstack(Vout)
    Vin  = np.vstack(Vin)

    def ring_side_faces(off, rings, segs, flip=False):
        F=[]
        for i in range(rings-1):
            a0 = off + i*segs
            a1 = off + (i+1)*segs
    ...
    rings = len(P); segs = segments
    Fout = ring_side_faces(0, rings, segs, flip=False)
    Fin  = ring_side_faces(len(Vout), rings, segs, flip=True)

    V = np.vstack([Vout, Vin])
    F_list = [Fout, Fin]

    # Annulus rim caps (watertight with a hole)
    # Start rim
    start_out0 = 0
    start_in0  = len(Vout)
    Fs=[]
    for j in range(segs):
        o0 = start_out0 + j
        o1 = start_out0 + (j+1)%segs
        i0 = start_in0 + j
        i1 = start_in0 + (j+1)%segs
        # two tris making the rim quad; winding chosen to keep outward normals
        Fs += [[o0, o1, i0], [i0, o1, i1]]
    # End rim
    end_out0 = (rings-1)*segs
    end_in0  = len(Vout) + (rings-1)*segs
    for j in range(segs):
        o0 = end_out0 + j
        o1 = end_out0 + (j+1)%segs
        i0 = end_in0 + j
        i1 = end_in0 + (j+1)%segs
        # opposite winding at end
        Fs += [[o0, i0, o1], [i0, i1, o1]]

    F_list.append(np.asarray(Fs, dtype=int))

    F = np.vstack(F_list)
    m = trimesh.Trimesh(vertices=V, faces=F, process=True)
    m.fix_normals()
    if not m.is_watertight:
        # Attempt light repair; should normally be watertight
        m.fill_holes()
...
def annotation_text(kind, p):
    ...
    return f"{kind} | w={p.w}µm | internal_d={p.internal_d}µm | outer+0.2mm{extra}"

def export_dxf_multi(kind, paths_um, p, base):
    doc=ezdxf.new(setup=True); msp=doc.modelspace()
    outer_mm=p.w/1000.0
    # stroke uses the outward-thickened outer diameter only
    outer2_mm,_=outward_thicken_params(outer_mm, 0.0, OUTWARD_THICKEN_MM)
...
def build_spiral(p):
    ...
    outer=um2mm(p.w)
    inner_d=um2mm(p.internal_d)
    outer2,_=outward_thicken_params(outer, 0.0, OUTWARD_THICKEN_MM)

    # Spiral has no junctions → build a watertight hollow *volume* directly (no boolean),
    # which avoids OpenSCAD boolean failures on tight spiral turns.
    mesh = sweep_hollow_tube_volume(path_mm, outer2, inner_d, segments=64)

    base=os.path.join(OUTDIR,"spiral_outward15")
    stl=f"{base}.stl"
    mesh.export(stl)
    print(f"✅ Spiral hollow STL → {os.path.abspath(stl)}")

    dxf=export_dxf_multi("Spiral", [path_um], p, base)
    svg=export_svg_local_multi("Spiral", [path_um], p, base)
    return stl,dxf,svg
...
def build_tjunction(p):
    ...
    inner_d=um2mm(p.internal_d)
    outer2,_=outward_thicken_params(outer, 0.0, OUTWARD_THICKEN_MM)
    # wall is derived from outer (after outward thickening) and internal diameter
    wall2=max((outer2 - inner_d)/2.0, 1e-6)
    ...
    canonicalize_stl_inplace(stl)

    dxf=export_dxf_multi("T-junction", [main_um, branch_um], p, base)
    svg=export_svg_local_multi("T-junction", [main_um, branch_um], p, base)
    return stl,dxf,svg
...
def build_yjunction(p):
    ...
    svg=export_svg_local_multi("Y-junction", [main_um, b1_um, b2_um], p, base)
    return stl,dxf,svg
...
def main():
    ...
    for fn,label in [(build_spiral,"Spiral"),(build_tjunction,"T-junction"),(build_yjunction,"Y-junction")]:
        ...
        rows.append({"session":session,"device":label,"t_s":f"{dt:.3f}","stl":stl,"dxf":dxf,"svg":svg,"stl_md5":canonical_md5(stl)})
